...
if("ggforce" %in% rownames(installed.packages())) {
  ...
  recentTickets <- tsTickets %>% filter(resolutionDate > (lubridate::now() - lubridate::days(30*2)))
...
```{r, echo=FALSE, warning=FALSE, error=FALSE}
recentTickets <- tsTickets %>% filter(resolutionDate > (lubridate::now() - lubridate::days(30*2))) %>% select(-one_of("secondsInColumns.Open", "secondsInColumns.Reopened", "secondsInColumns.Resolved", "secondsInColumns.Closed"))
if (nrow(recentTickets)) {
