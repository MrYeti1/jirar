...
workedColumns <- tsTickets %>% select(starts_with("SecondsInColumn")) %>%
  select( -one_of(c("secondsInColumns.Work.Backlog", "secondsInColumns.Backlog", "secondsInColumns.Next.Up", "secondsInColumns.New", "secondsInColumns.Done", "secondsInColumns.Closed", "secondsInColumns.Resolved", "secondsInColumns.Open", "secondsInColumns.Reopened")))
...
```{r}
ggplot(filter(tsTickets, ! is.na(resolutionDate)) %>% filter(workTime < quantile(workTime, 0.95)), aes(x=storyPoints, group=storyPoints, y=workTime/24/60/60/1000)) + geom_violin() + ggbeeswarm::geom_quasirandom(alpha=0.3, size=0.5) + ggtitle("Story Size vs Work Time", subtitle("Excluding longest 5% of tickets")) + ylab("Days")
ggplot(filter(tsTickets, ! is.na(resolutionDate)), aes(x=storyPoints, group=storyPoints, y=totalTime/24/60/60)) + geom_violin() + ggbeeswarm::geom_quasirandom(alpha=0.3, size=0.5)
```
...
library(forcats)
  ...
  classOfWorkTickets$spendDisplay <- fct_relevel(fct_explicit_na(addNA(classOfWorkTickets$spend), "NA"), "NA", "OPEX", "CAPEX")
...
#    ggtitle("Spend Distribution - Completed Tickets") + xlab("Resolution Month") + ylab("Spend %")
    ...
    ggplot(filter(classOfWorkTickets, ! is.na(resolutionMonthDisplay)), aes(x=resolutionMonthDisplay, y=workTime, fill=spendDisplay)) + scale_fill_manual(name=NULL, values=c("OPEX"="#F8766D", "CAPEX"="#00BFC4", "NA"="grey50")) +
