...
tsTickets <- read.csv(datasetFile, header=T, skipNul=T, na.strings="") %>%
  mutate(
    createdDate = as.POSIXct(created), 
    ...
    totalTime = resolutionDate - createdDate
  ) %>% 
...
workedColNames = c("In.Progress", "In.Dev")
#select( -one_of(c("secondsInColumns.Backlog", "secondsInColumns.Next.Up", "secondsInColumns.New", "secondsInColumns.Done", "secondsInColumns.Closed", "secondsInColumns.Resolved", "secondsInColumns.Open", "secondsInColumns.Reopened", "secondsInColumns.Selected.for.Development")))

workedColumns <- tsTickets %>% 
  select(starts_with("SecondsInColumn")) %>%
  select(one_of(paste0("secondsInColumns.", workedColNames)))

leftWorkedColumns <- tsTickets %>% 
  select(starts_with("leftColumnDate")) %>%
  select(one_of(paste0("leftColumnDate.", workedColNames)))
...
statusLabels <- gsub("secondsInColumns.", "", names(workedColumns))
names(statusLabels) <- names(workedColumns)

load(file="colColourFit.Rdata")
load(file="colColourTdm.Rdata")

guessedStatusColours <- guessColumnColours(names(statusLabels), colColourFit, colColourTdm)
...
controlColumns <- bind_cols(select(tsTickets, key, resolutionDate), workedColumns) %>%
  ...
  filter( resolutionDate > ymd(Sys.Date(), tz="Europe/London") %m-% months(3) )

if (nrow(controlColumns) > 0) {
  #Melt for plotting
  workedMelt <- melt(controlColumns, id=c("key", "resolutionDate")) %>% 
    mutate(variable = gsub("secondsInColumns.", "", variable))

  leftCols <- bind_cols(select(tsTickets, key, resolutionDate), leftWorkedColumns)
  leftMelt <- melt(leftCols, id=c("key", "resolutionDate"), value.name = "leaveDate") %>%
    mutate(variable = gsub("leftColumnDate.", "", variable))

  leavingDateMelt <- left_join(workedMelt, leftMelt, by=c('key', "resolutionDate", "variable"))
  ...
  ggplot(workedMelt, aes(x=resolutionDate, y=value/1000/60/60/24, color=paste0("secondsInColumns.",variable))) + geom_point() + ylab("Hours") + xlab("Resolution Date") + facet_wrap(~variable, ncol=2, scales="free_y") +
    scale_color_manual(values=guessedStatusColours, na.value="gray", labels=statusLabels, name="Column") + theme(legend.position="None") + geom_smooth(na.rm = T, se=T) +
...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=F, fig.height=6}
if (nrow(controlColumns) > 0) { 
 ggplot(leavingDateMelt, aes(x=as.POSIXct(leaveDate), y=value/1000/60/60/24, color=paste0("secondsInColumns.",variable))) + geom_point() + ylab("Hours") + xlab("Left Column Date") + facet_wrap(~variable, ncol=2, scales="free_y") +
    scale_color_manual(values=guessedStatusColours, na.value="gray", labels=statusLabels, name="Column") + theme(legend.position="None") + geom_smooth(na.rm = T, se=T) +
    scale_y_continuous(labels=prettyNum)
...
```{r, echo=F}
  ...
  suppressWarnings(classOfWorkTickets$spendDisplay <- fct_relevel(fct_explicit_na(addNA(classOfWorkTickets$spend), "NA"), "NA", "OPEX", "CAPEX"))
...
#    ggtitle("Spend Distribution - Completed Tickets") + xlab("Resolution Month") + ylab("Spend %")
    ...
    ggplot(filter(classOfWorkTickets, ! is.na(resolutionMonthDisplay)), aes(x=resolutionMonthDisplay, y=workTime, fill=spendDisplay)) + scale_fill_manual(name=NULL, values=c("OPEX"="#F8766D", "CAPEX"="#00BFC4", "NA"="grey50", "Known"="#fc8d62", "Mixed"="#8da0cb", "Unknown"="#66c2a5")) +
