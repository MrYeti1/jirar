...
statusColours <- c("secondsInColumns.Open" = "white", "secondsInColumns.Backlog" = "white",
                   ...
                   "secondsInColumns.Closed" = "X666666",
                   "secondsInColumns.Resolved" = "X666666",
                   "secondsInColumns.Done" = "X666666"
...
trainRatio <- 0.75
trainSize <- floor(length(statusColours) * 0.75)
set.seed(1331)
...
fit <- caret::train(colour ~ ., data = statusTDM.train, 
                  method = "svmLinear",
                  #metric = "ROC",
                  preProc = c("zv"),# "center", "scale"),
                  trControl = ctrl)
predict(fit, newdata = statusTDM.train, type="raw")
#fit <- caret::train(colour ~ ., data=statusTDM.train, method = "bayesglm", trControl = ctrl)


statusTDM.pred <- predict(fit, newdata = statusTDM.train, type="raw")
#factor(possibleColours[round(abs(predict(fit, newdata = statusTDM.train[, !(colnames(statusTDM.train) == "colour")])))], levels=possibleColours)
...
statusTDM.train <- as.data.frame(statusTDM.train)
statusTDM.train$colour <- factor(statusColours.train$colour, levels=possibleColours)
statusTDM.pred.test <- factor(predict(fit, newdata=as.matrix(tdm.test)), levels=possibleColours)

caret::confusionMatrix(statusTDM.pred.test, statusColours.test$colour)
resultTable <- table(
  orig=statusColours.test$colour,
