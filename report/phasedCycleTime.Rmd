...
tsTickets <- read.csv(datasetFile, header=T, skipNul=T, na.strings="") %>%
  ...
  select( -totalTime, -spend, -one_of(c("secondsInColumns.Open", "secondsInColumns.New", "secondsInColumns.Next.Up", "secondsInColumns.Backlog", "secondsInColumns.Work.Backlog", "secondsInColumns.Resolved", "secondsInColumns.Reopened", "secondsInColumns.Closed", "secondsInColumns.Done", "secondsInColumns.Coming.Next"))) 
...
dwellPerc <- summaryResolvedMelt %>% filter(!is.na(resolutionMonth)) %>% mutate( dwell = grepl(".done|.out|release.validation|Ready.For.", variable, ignore.case=T)) %>% 
...
guessedStatusColours <- guessColumnColours(summaryResolvedMelt$variable %>% unique(), colColourFit, colColourTdm)

statusLabels <- gsub("secondsInColumns.", "", names(guessedStatusColours))
names(statusLabels) <- names(guessedStatusColours)
numLabelRows <- (names(guessedStatusColours) %>% stringi::stri_length() %>% sum() / 130) %>% ceiling()
...
summaryResolvedMelt %>%
  ...
  theme(legend.position = "bottom") + scale_fill_manual(values=guessedStatusColours, na.value="gray", labels=statusLabels, name="Column") + coord_flip() + scale_x_discrete(labels=dateDwell) + expand_limits(0) +
  guides(fill=guide_legend(reverse=TRUE,title=NULL, byrow=F)) + ggtitle("Median Phased Cycle Time - Resolved Date")
...
dwellPercMean <- summaryResolvedMelt %>% filter(!is.na(resolutionMonth)) %>% mutate( dwell = grepl(".done|.out|release.validation|Ready.For.", variable, ignore.case=T)) %>% 
  group_by(resolutionMonth, resolutionMonthDisplay, dwell) %>% 
  summarise(total=sum(value, na.rm=T)) %>% mutate( perc = round(total/sum(total)*100)) %>% filter(dwell) %>% select(resolutionMonth, resolutionMonthDisplay, perc)
...
summaryResolvedMelt %>%
  ...
  ggplot(aes(x=resolutionMonthDisplay, y=value/24/60/60/1000)) +
  geom_bar(stat="identity", aes(fill=variable)) + ylab("days") + xlab("month") +
  theme(legend.position = "bottom", legend.justification = "left") + scale_fill_manual(values=guessedStatusColours, na.value="gray", labels=statusLabels, name="Column") + coord_flip() + scale_x_discrete(labels=dateDwellMean) +
  guides(fill=guide_legend(reverse=TRUE,title=NULL,byrow=F)) + ggtitle("Mean Phased Cycle Time - Resolved Date")
```
